-- ============================================
-- GENERATE SAMPLE DATA (BASIC VERSION)
-- This works with just the base schema (schema.sql)
-- No extended tables required
-- ============================================
...
BEGIN
    ...
    -- Insert sample posts (3-4 posts per user = ~175 posts)
    FOR i IN 1..50 LOOP
        -- Post 1
    ...
    RAISE NOTICE '✅ Created ~175 sample posts';
    
    -- Create some likes (200 random likes)
    FOR i IN 1..200 LOOP
    ...
    RAISE NOTICE '✅ Created ~200 sample likes';
    
    -- Create some comments (100 random comments)
    FOR i IN 1..100 LOOP
        random_user_id := sample_user_ids[1 + floor(random() * 50)::int];
        SELECT id INTO random_post_id FROM public.posts ORDER BY random() LIMIT 1;
    ...
    RAISE NOTICE '✅ Created ~100 sample comments';
